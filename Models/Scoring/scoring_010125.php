...
$sqlFS = "SELECT SUM(lost_point) AS total FROM audit_point WHERE audit_id = $audit_id AND section_number<=7 ";
$foodSafety = $res->select($sqlFS);
//$foodSafety = $this->select($sqlFS);

$sqlBS = "SELECT SUM(lost_point) AS total FROM audit_point WHERE audit_id = $audit_id AND section_number>7 ";
$operations = $res->select($sqlBS);
//$operations = $this->select($sqlBS);

#### Obtener los puntos NA para ajustar los targets--------------------------------------------
$sqlPtsNA = "SELECT (SELECT SUM(target_point) FROM audit_na_question WHERE audit_id=$audit_id AND section_number<=7) as totalFS, (SELECT SUM(target_point) FROM audit_na_question WHERE audit_id=$audit_id AND section_number>7) as totalBS, (SELECT SUM(target_point) FROM audit_na_question WHERE audit_id=$audit_id) as total FROM audit_na_question LIMIT 1 ";
...
$sqlTarget = "SELECT (SELECT SUM(points) FROM checklist_item WHERE checklist_id=$audit[checklist_id] AND section_number<=7 $areas) as totalFS, (SELECT SUM(points) FROM checklist_item WHERE checklist_id=$audit[checklist_id] AND section_number>7 $areas) as totalBS, (SELECT SUM(points) FROM checklist_item WHERE checklist_id=$audit[checklist_id] $areas) as total FROM checklist_item LIMIT 1 ";
$target = $res->select($sqlTarget);

#### Identificar puntos criticos en el checklist--------------------------------------------
$sqlPCrit = "SELECT GROUP_CONCAT(id SEPARATOR ',') AS ids FROM checklist_item WHERE checklist_id=$audit[checklist_id] AND priority='First Priority' ";
...
$sqlPAF = "SELECT GROUP_CONCAT(id SEPARATOR ',') AS ids2 FROM checklist_item WHERE checklist_id=$audit[checklist_id] AND AutoFail='Auto Fail' ";
...
if(empty($operations['total'])) $operations['total']=0;
$totalPerdidos = $foodSafety['total'] + $operations['total'];

$overallFS = 100-$foodSafety['total'];
$overallFS = number_format($overallFS, 2);
$operationsE = 100-$operations['total'];
$operationsE = number_format($operationsE, 2);
$overallScore = 100-$totalPerdidos;
...
if($score['id']>0){
	...
	$request = $res->update($sql_update, [$overallFS, $operationsE, $letra, $overallScore, $audit_id]);
	...
	$request = $res->insert($sql_insert, [$audit_id, 'General', 'OVERALL SCORE', $overallFS, $operationsE, $letra, $overallScore]);
...
if($request){
	...
	$result['OperationsE'] = $operationsE;
	$result['Letra'] = $letra;
	$result['color'] = $color;
